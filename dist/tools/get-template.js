import { generateSetup, generateBeforeAfter, generateTargetFunctions, generateProperties, generateCryticTester, } from "../templates/base.js";
import { generateFoundryToml, generateEchidnaYaml, generateMedusaJson, } from "../templates/configs.js";
...
export function getTemplate(input) {
    const templateName = input.template_name.toLowerCase();
    const config = templateConfigs[templateName];
    if (!config) {
        const validNames = Object.keys(templateConfigs).join(", ");
        throw new Error(`Unknown template: "${input.template_name}". Valid templates: ${validNames}`);
    }
