...
export const governanceProperties: PropertyDef[] = [
  {
    name: "invariant_vote_weight_conservation",
    description:
      "Total vote weight across all voters must equal the total voting power supply (no votes created from nothing)",
    severity: "critical",
  ...
  },
  {
    ...
    description:
      "Quorum threshold must not decrease during an active proposal's voting period",
    severity: "high",
  ...
  },
  {
    ...
    description:
      "Proposals must follow valid state transitions: Pending -> Active -> Succeeded/Defeated -> Queued -> Executed",
    severity: "critical",
  ...
  },
  {
    ...
    description:
      "Execution must only occur after the timelock delay has elapsed and before the grace period expires",
    severity: "critical",
  ...
  },
  {
    ...
    description:
      "An address must not be able to cast votes more than once on the same proposal",
    severity: "critical",
  ...
  },
  {
    ...
    description:
      "Delegating votes must not create or destroy voting power, only transfer it",
    severity: "high",
  ...
  },
  {
    ...
    description:
      "A cancelled proposal must never transition to Queued or Executed state",
    severity: "high",
  ...
  },
  {
    ...
    description:
      "forVotes + againstVotes + abstainVotes must equal total votes cast on a proposal",
    severity: "medium",
  ...
  },
  {
    ...
    description:
      "Only addresses with voting power >= proposalThreshold can create proposals",
    severity: "high",
  ...
  },
  {
    ...
    description:
      "An executed proposal's actions must have been applied exactly once",
    severity: "critical",
